...
def get_max_episode_steps(env):
    ...
    if hasattr(env, '_max_episode_steps'):
        ...
        return get_max_episode_steps(env.envs[0].unwrapped)
    else:
        return 1000  # 默认值
...
if __name__ == '__main__':
    ...
    # 设置评估环境
    eval_env = SubprocVecEnv([make_env(args.env_id, record_dir=None if "eval" in args.exp_name else "logs/videos") 
    ...
    eval_callback = EvalCallback(eval_env, 
                                best_model_save_path="./logs/" if "eval" not in args.exp_name else None,
                                log_path="./logs/" if "eval" not in args.exp_name else None,
    ...
    model.learn(args.train_max_steps, callback=callbacks)
    ...
    if "eval" not in args.exp_name:
        ...
        # 评估并保存视频
        returns, ep_lens = evaluate_policy(model, eval_env, deterministic=True, render=False,
                                         return_episode_rewards=True, n_eval_episodes=20)
    ...
    # 计算并打印成功率
    success = np.array(ep_lens) < get_max_episode_steps(eval_env)

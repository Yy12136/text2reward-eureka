...
def compute_dense_reward(self, action) -> float:
    ...
    # 21. Encourage minimal deviation from the initial gripper orientation
    if is_grasped and not is_cubeA_at_goal_height:
        initial_gripper_orientation = self.robot.ee_pose.q
    ...
    # 22. Encourage minimal deviation from the initial cube orientation
    if is_grasped and not is_cubeA_at_goal_height:
    ...
    # 23. Encourage minimal deviation from the initial cube position
    if is_grasped and not is_cubeA_at_goal_height:
        initial_cube_position = self.cubeA.pose.p
        deviation_from_initial_cube_position = np.linalg.norm(initial_cube_position - self.cubeA.pose.p)
        deviation_from_initial_cube_position_penalty = -0.2 * deviation_from_initial_cube_position
        reward += deviation_from_initial_cube_position_penalty

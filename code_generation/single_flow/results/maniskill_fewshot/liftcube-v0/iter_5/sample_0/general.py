...
def compute_sparse_reward(self, action) -> float:
    ...
    if tcp_to_cubeA_dist < 0.02:  # Threshold for reaching the cube
    ...
    if is_grasped:
        ...
        if cubeA_height >= self.goal_height - 0.01:  # Threshold for lifting to the goal height
            reward += 5.0  # Reward for lifting the cube to the goal height

    # 4. Task completion (cube is stable at the goal height)

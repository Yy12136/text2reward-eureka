...
def compute_dense_reward(self, action) -> float:
    ...
    if is_grasped:
        ...
        # Milestone: Reward for maintaining a stable grasp during opening
        gripper_position_diff = np.linalg.norm(ee_pose.p - self.cabinet.handle.pose.p)
        grasp_stability_reward = 1 - np.tanh(5.0 * gripper_position_diff)
        reward += grasp_stability_reward
        ...
        # Penalize abrupt changes in joint velocities

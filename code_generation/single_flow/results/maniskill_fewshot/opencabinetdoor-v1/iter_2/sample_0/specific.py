...
def compute_dense_reward(self, action) -> float:
    ...
    if is_door_open:
        reward += 20.0  # Large reward for task completion
        return reward
    ...
    if dist_to_handle < 0.05:
        reward += 2.0  # Milestone reward for being close to the handle
    ...
    if is_grasped:
        reward += 3.0  # Reward for successful grasp

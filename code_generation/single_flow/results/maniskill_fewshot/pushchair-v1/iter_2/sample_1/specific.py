...
def compute_dense_reward(self, action) -> float:
    ...
    # Penalize if the chair is too far from the target
    if chair_to_target_dist > 1.0:
        reward -= 1.0

    # Penalize if the chair is falling (tilt angle too large)
    if chair_tilt > np.pi / 6:  # 30 degrees
        reward -= 1.0

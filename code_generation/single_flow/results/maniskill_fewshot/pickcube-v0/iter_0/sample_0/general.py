...
def compute_sparse_reward(self, action) -> float:
    ...
    if is_grasped:
        # Reward for placing cube A at the goal
        if is_cubeA_at_goal and is_cubeA_static:
            reward += 50.0  # Large reward for placing the cube at the goal

    # Penalize dropping the cube
    if is_grasped and not self.robot.check_grasp(self.cubeA, max_angle=30):
        reward -= 30.0  # Significant penalty for dropping the cube

    # Penalize excessive action magnitude

...
def compute_sparse_reward(self, action) -> float:
    ...
    if not is_grasped:
        ...
        if is_grasped:
            reward += 20.0  # Significant reward for grasping the cube

    # Stage 2: Moving Cube A to Goal
    if is_grasped:
        # Reward for reducing the distance to the goal
        if cubeA_to_goal_dist < 0.1:  # Threshold for proximity to the goal
            reward += 30.0  # Significant reward for getting close to the goal

    # Stage 3: Releasing Cube A at Goal
    if is_grasped and cubeA_to_goal_dist < 0.05:
        # Reward for releasing the cube at the goal
        if not is_grasped and is_cubeA_at_goal:
            reward += 50.0  # Large reward for successfully releasing the cube at the goal

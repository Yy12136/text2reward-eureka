...
def compute_dense_reward(self, action) -> float:
    ...
    # Success reward
    if success:
        reward += 100.0  # Increased success reward for completing the task
        return reward
    ...
    if not is_grasped:
        ...
        reward += 3.0 * gripper_openness_reward  # Increased weight for gripper openness

        # Penalize excessive gripper openness
        if gripper_openness > 0.7:
            reward -= 2.0  # Increased penalty for excessive openness

    # Stage 2: Grasping Cube A
    if not is_grasped:
        # Additional reward for successful grasp
        if is_grasped:
            reward += 20.0  # Increased reward for successful grasp
        else:
            # Penalize unsuccessful grasp attempts
            reward -= 5.0  # Increased penalty for unsuccessful grasp attempts
    ...
    if is_grasped:
        ...
        if not is_grasped:
            reward -= 30.0  # Increased penalty for dropping the cube

        # Reward maintaining a stable grasp
        if is_grasped:
            reward += 3.0  # Increased reward for maintaining a stable grasp
    ...
    if is_grasped:
        ...
        if cubeA_to_goal_dist > 0.5:
            reward -= 20.0  # Increased penalty for moving away from the goal

        # Reward maintaining a stable grasp while moving
        if is_grasped:
            reward += 3.0  # Increased reward for maintaining a stable grasp while moving
    ...
    if is_grasped and cubeA_to_goal_dist < 0.05:
        ...
        reward += 20.0 * gripper_openness_reward  # Increased reward for correct gripper openness

        # Penalize releasing the cube too early or too late
        if gripper_openness < 0.9:
            reward -= 15.0  # Increased penalty for incorrect gripper openness

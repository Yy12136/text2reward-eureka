...
def compute_dense_reward(self, action) -> float:
    ...
    # Success reward
    if success:
        reward += 100.0  # Higher reward for task completion
        return reward
    ...
    if not is_grasped:
        ...
        reward += 2.0 * reaching_reward  # Increased weight for reaching

        # Encourage gripper openness for grasping
        gripper_openness = self.robot.gripper_openness
        gripper_openness_reward = 1 - np.abs(gripper_openness - 0.5)  # Encourage half-open gripper
    ...
    if not is_grasped:
        ...
        if is_grasped:
            reward += 20.0  # Higher reward for successful grasp
        else:
    ...
    if is_grasped:
        ...
        reward += 3.0 * lifting_reward  # Increased weight for lifting
    ...
    if is_grasped:
        ...
        reward += 3.0 * moving_reward  # Increased weight for moving
    ...
    if is_grasped and cubeA_to_goal_dist < 0.05:
        ...
        reward += 20.0 * gripper_openness_reward  # Higher reward for releasing

...
def compute_sparse_reward(self, action) -> float:
    ...
    if not self.robot.check_grasp(self.cubeA, max_angle=30):  # If Cube A is not grasped
        ...
        # Step 2: Lift Cube A
        if self.cubeA.pose.p[2] > self.cubeB.pose.p[2] + self.cube_half_size:  # If Cube A is lifted
            ...
            if cubeA_to_cubeB_dist < 0.05:  # If Cube A is close to Cube B
                if self.cubeA.check_static() and not self.robot.check_grasp(self.cubeA, max_angle=30):  # Stable placement
                    reward += 2.0  # Reward for stable placement

    # Step 4: Penalize failure to complete the task
    if self.current_step == self.max_steps and not (self.cubeA.check_static() and not self.robot.check_grasp(self.cubeA, max_angle=30)):
        reward -= 1.0  # Penalize for not completing the task

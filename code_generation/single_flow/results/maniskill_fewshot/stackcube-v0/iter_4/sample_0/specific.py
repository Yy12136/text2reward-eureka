...
def compute_dense_reward(self, action) -> float:
    ...
    # Step 16: Penalize unnecessary movements of the gripper when Cube A is not grasped
    if not self.robot.check_grasp(self.cubeA, max_angle=30):
        reward += -0.01 * np.linalg.norm(self.robot.ee_pose.p - self.cubeA.pose.p)  # Penalize unnecessary movements

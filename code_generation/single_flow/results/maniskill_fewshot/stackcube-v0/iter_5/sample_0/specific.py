...
def compute_dense_reward(self, action) -> float:
    ...
    # Step 17: Reward for minimizing the height difference between Cube A and Cube B during placement
    if self.robot.check_grasp(self.cubeA, max_angle=30):
        height_diff = abs(self.cubeA.pose.p[2] - (self.cubeB.pose.p[2] + 2 * self.cube_half_size))
        reward += -0.1 * height_diff  # Penalize height difference during placement

...
def compute_dense_reward(self, action) -> float:
    ...
    # Step 12: Reward for minimizing the distance between gripper and Cube A during lifting
    if self.robot.check_grasp(self.cubeA, max_angle=30):
        gripper_to_cubeA_dist = np.linalg.norm(self.robot.ee_pose.p - self.cubeA.pose.p)
        reward += -0.1 * gripper_to_cubeA_dist  # Penalize distance to Cube A during lifting

    # Step 13: Reward for minimizing the distance between gripper and Cube B during placement
    if self.robot.check_grasp(self.cubeA, max_angle=30):
        gripper_to_cubeB_dist = np.linalg.norm(self.robot.ee_pose.p - self.cubeB.pose.p)
        reward += -0.1 * gripper_to_cubeB_dist  # Penalize distance to Cube B during placement

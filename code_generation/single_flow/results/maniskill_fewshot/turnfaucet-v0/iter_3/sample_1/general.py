...
def compute_sparse_reward(self, action) -> float:
    ...
    if min_dist < 0.1:  # Threshold for being close enough to the handle
        reward += 1.0  # Reward for being close to the handle
    ...
    if is_grasped:
        ...
        if rotation_progress > 0.5:  # Threshold for significant progress
            reward += 2.0  # Reward for making significant progress in rotation

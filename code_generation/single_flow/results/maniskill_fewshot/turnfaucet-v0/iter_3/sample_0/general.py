...
def compute_sparse_reward(self, action) -> float:
    ...
    # Reward only when the gripper is close enough to the handle
    ...
    if min_dist < 0.05:  # Threshold for approach
        reward += 1.0  # Reward for successful approach
    ...
    if is_grasped:
        # Reward based on the progress in rotating the handle
        rotation_progress = current_qpos / target_qpos
        if rotation_progress > 0.5:  # Threshold for significant progress
            reward += 3.0  # Reward for significant rotation progress

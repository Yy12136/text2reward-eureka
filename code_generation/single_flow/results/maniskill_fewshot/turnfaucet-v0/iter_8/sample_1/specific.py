...
def compute_dense_reward(self, action) -> float:
    ...
    # Additional Milestone: Ensure the gripper is oriented correctly towards the handle
    gripper_pose = self.robot.ee_pose
    handle_pose = self.faucet.handle.pose
    gripper_to_handle = handle_pose.inv() * gripper_pose
    # Calculate the angle between the gripper's z-axis and the handle's z-axis
    ...
    # Additional Milestone: Encourage the gripper to approach the handle from the correct side
    # Calculate the vector from the gripper to the handle
    gripper_to_handle_vector = handle_pose.p - gripper_pose.p
    # Normalize the vector
    gripper_to_handle_vector = gripper_to_handle_vector / np.linalg.norm(gripper_to_handle_vector)
    # Calculate the dot product between the gripper's z-axis and the vector to the handle
    approach_direction = np.dot(gripper_pose.to_transformation_matrix()[:3, 2], gripper_to_handle_vector)
    approach_direction_reward = np.tanh(5 * approach_direction)
    reward += approach_direction_reward

    # Additional Milestone: Encourage the gripper to maintain a stable position relative to the handle
    # Calculate the distance between the gripper and the handle after grasping
    if is_grasped:
    ...
    # Calculate the deviation from the robot's neutral joint configuration
    ...
    # Calculate the velocity of the robot's joints
    ...
    # Calculate the change in gripper openness
    ...
    # Calculate the deviation from the robot's neutral end-effector position
    ...
    # Calculate the deviation from the robot's neutral end-effector orientation
    neutral_ee_orientation = np.array([0.0, 0.0, 0.极, 1.0])  # Assuming neutral end-effector orientation

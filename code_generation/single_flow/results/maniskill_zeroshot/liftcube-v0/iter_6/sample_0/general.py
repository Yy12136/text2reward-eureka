    ...
    if self.agent.check_grasp(self.obj, max_angle=30):
        ...
        if height_difference < 0.01:  # Threshold for reaching the goal height
            ...
            if self.obj.check_static():
                reward = 1.0  # Large reward for successful task completion
            else:
                reward = -0.5  # Penalize instability
        else:
            reward = -0.2  # Penalize not reaching the goal height
    else:
        reward = -0.1  # Penalize not grasping Cube A

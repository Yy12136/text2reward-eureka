    ...
    if not self.agent.check_grasp(self.obj, max_angle=30):
        ...
        if height_difference < 0.01:  # Threshold for reaching the goal height
            ...
            if check_actor_static(self.obj):
                # Reward for stabilizing Cube A
                reward += 1.0
            else:
                # Penalize instability
                reward -= 0.5
        else:
            # Penalize not lifting Cube A to the goal height
            reward -= 0.5

    # Step 4: Penalize dropping Cube A during lifting
    if self.agent.check_grasp(self.obj, max_angle=30) and self.obj.pose.p[2] < self.goal_height:
        reward -= 1.0

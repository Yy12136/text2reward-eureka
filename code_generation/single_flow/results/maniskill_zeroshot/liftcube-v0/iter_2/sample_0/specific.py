...
def compute_dense_reward(self, action) -> float:
    ...
    # Reward for reducing the distance to cube A (smooth transition)
    ...
    if distance_to_cubeA < 0.05:  # If gripper is close enough to cube A
        ...
        if is_grasped:
            reward += 1.0
            # Additional reward for grasp quality (alignment of gripper with cube)
            gripper_orientation = self.tcp.pose.q
            cubeA_orientation = self.obj.pose.q
            orientation_diff = np.linalg.norm(gripper_orientation - cubeA_orientation)
            reward += max(0, 1 - orientation_diff / np.pi) * 0.5  # Weight: 0.5
        else:
    ...
    if self.agent.check_grasp(self.obj, max_angle=30):  # If cube A is grasped
        ...
        reward += max(0, 1 - height_difference / 0.2) * 1.0  # Weight: 1.0
        
        # Additional reward for maintaining a stable grasp while lifting
        if height_difference < 0.1:  # If close to the goal height
            reward += 0.5  # Weight: 0.5
    
    # Regularization of the robot's action
    # Penalize large actions to encourage smooth movements
    action_penalty = -0.1 * np.linalg.norm(action)  # Increased weight: 0.1

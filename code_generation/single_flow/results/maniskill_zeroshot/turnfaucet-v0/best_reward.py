...
def compute_dense_reward(self, action) -> float:
    ...
    # Stage 3: Open the gripper to prepare for grasping
    # Reward for opening the gripper when close to the handle
    if min_distance < 0.1:
        gripper_openness = self.agent.robot.get_qpos()[-1] / self.agent.robot.get_qlimits()[-1, 1]
        reward += 0.1 * gripper_openness  # Encourage the gripper to open

    # Stage 4: Grasp the handle

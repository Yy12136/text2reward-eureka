...
def compute_dense_reward(self, action) -> float:
    ...
    # Stage 7: Dual-Arm Coordination
    # Calculate the distance between the two end-effectors
    ee_dist = np.linalg.norm(ee_coords[0] - ee_coords[1])
    # Reward for keeping the end-effectors at an optimal distance for pushing
    reward += -0.1 * np.abs(ee_dist - 0.2)  # Optimal distance is 0.2 meters

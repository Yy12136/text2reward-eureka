...
def compute_dense_reward(self, action) -> float:
    ...
    # Stage 10: Chair Final Position
    # Ensure the chair is exactly at the target position
    if dist_to_target < 0.05:
        reward += 1.0

    # Stage 11: Chair Force Control
    # Penalize excessive force applied to the chair
    applied_force = np.linalg.norm(self.robot.get_applied_force())
    reward += -0.1 * applied_force

    # Stage 12: Chair Collision Avoidance
    # Penalize any collisions between the chair and the robot
    if self.check_collision(self.root_link, self.robot):
        reward += -0.5

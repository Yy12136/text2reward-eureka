...
def compute_sparse_reward(self, action) -> float:
    ...
    # Check if the robot is close enough to the chair
    ...
    # Check if the chair is stable (not tilted or rotating)
    ...
    if chair_tilt < 0.1 and chair_angular_velocity < 0.1:  # Threshold for stability
        reward += 1.0  # Sparse reward for keeping the chair stable
    ...
    if dist_to_target < 0.1 and chair_tilt < 0.1 and chair_angular_velocity < 0.1:
        reward += 5.0  # Sparse reward for completing the task

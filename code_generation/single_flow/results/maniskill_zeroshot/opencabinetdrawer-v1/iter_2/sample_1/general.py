...
def compute_sparse_reward(self, action) -> float:
    ...
    # Reward for closing the gripper sufficiently
    if gripper_openness < 0.1:  # Threshold for a successful grasp
        reward += 1.0  # Milestone reward for grasping the handle
    
    # Stage 3: Pull the Drawer
    # Reward for partially opening the drawer
    if drawer_qpos > 0.1 * target_qpos:  # Threshold for starting to pull
        reward += 1.0  # Milestone reward for initiating the pull
    
    # Stage 4: Task Completion
    # Large reward for fully opening the drawer

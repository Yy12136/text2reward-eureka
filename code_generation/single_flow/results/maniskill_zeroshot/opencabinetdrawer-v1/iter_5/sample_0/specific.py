...
def compute_dense_reward(self, action) -> float:
    ...
    # Milestone reward for nearing completion
    if drawer_qpos > 0.8 * target_qpos:  # If the drawer is almost fully opened
        reward += 3.0  # Milestone reward for nearing completion
    ...
    # Penalize if the gripper is not aligned with the handle when pulling
    if drawer_qpos > 0.1 * target_qpos and np.linalg.norm(ee_pose.p - handle_pose.p) > 0.05:

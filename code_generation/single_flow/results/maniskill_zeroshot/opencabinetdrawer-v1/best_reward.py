...
def compute_dense_reward(self, action) -> float:
    ...
    base_movement_penalty = -0.05 * np.linalg.norm(self.agent.base_link.velocity[:2])
    ...
    # Penalize if the robot is not facing the cabinet
    robot_to_cabinet_vector = self.cabinet.pose.p[:2] - self.agent.base_pose.p[:2]
    robot_forward_vector = np.array([np.cos(ee_pose.q[0]), np.sin(ee_pose.q[0])])
    ...
    if angle_diff > np.pi / 4:  # If the robot is not facing the cabinet within 45 degrees
        reward -= 0.5  # Penalize for not facing the cabinet

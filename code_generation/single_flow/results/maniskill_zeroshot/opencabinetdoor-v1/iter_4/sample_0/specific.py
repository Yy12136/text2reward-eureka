...
def compute_dense_reward(self, action) -> float:
    ...
    # Penalize excessive base movement to encourage efficient navigation
    base_velocity_penalty = -0.01 * np.linalg.norm(self.agent.base_velocity)
    reward += base_velocity_penalty

    # Penalize excessive joint velocities to encourage smooth movements
    joint_velocity_penalty = -0.01 * np.linalg.norm(self.agent.robot.qvel)
    reward += joint_velocity_penalty

    # Penalize excessive gripper movement to encourage precise grasping
    gripper_velocity_penalty = -0.01 * abs(self.agent.robot.qvel[-1])

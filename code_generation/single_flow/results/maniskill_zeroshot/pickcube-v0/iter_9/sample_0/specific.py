...
def compute_dense_reward(self, action) -> float:
    ...
    if not self.agent.check_grasp(self.obj) and np.linalg.norm(self.obj.pose.p - self.goal_pos) < 0.01 and check_actor_static(self.obj):
        reward += 1.0  # Reward for Cube A being static at the goal

    # Additional milestone: Cube A is correctly oriented at the goal
    if not self.agent.check_grasp(self.obj) and np.linalg.norm(self.obj.pose.p - self.goal_pos) < 0.01:
        # Calculate the angle difference between the current orientation and the goal orientation
        goal_orientation = np.array([1, 0, 0, 0])  # Assuming the goal orientation is aligned with the world frame
        current_orientation = self.obj.pose.q
        angle_diff = np.arccos(np.abs(np.dot(goal_orientation, current_orientation)))
        orientation_reward = 1.0 - angle_diff / np.pi  # Reward for correct orientation

import numpy as np

def compute_sparse_reward(self, action) -> float:
    # Initialize reward
    reward = 0.0

    # Step 1: Grasp Success
    if self.agent.check_grasp(self.obj):
        reward += 1.0  # Reward for successful grasp

    # Step 2: Lift Cube
    if self.agent.check_grasp(self.obj) and self.obj.pose.p[2] > 0.05:  # Cube is lifted above a threshold
        reward += 1.0  # Reward for lifting the cube

    # Step 3: Place at Goal
    if not self.agent.check_grasp(self.obj) and np.linalg.norm(self.obj.pose.p - self.goal_pos) < 0.01:
        reward += 2.0  # Reward for placing the cube at the goal position

    # Penalties
    if not self.agent.check_grasp(self.obj) and np.linalg.norm(self.obj.pose.p - self.goal_pos) > 0.1:
        reward -= 1.0  # Penalize dropping the cube away from the goal

    return reward
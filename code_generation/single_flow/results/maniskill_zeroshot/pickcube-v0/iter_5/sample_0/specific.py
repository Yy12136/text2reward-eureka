...
def compute_dense_reward(self, action) -> float:
    ...
    # Additional Milestone: Orient Cube A correctly (optional, if orientation matters)
    if not self.agent.check_grasp(self.obj) and np.linalg.norm(self.obj.pose.p - self.goal_pos) < 0.01:
        goal_orientation = self.goal_orientation  # Assuming goal_orientation is defined
        current_orientation = self.obj.pose.q
        orientation_diff = np.linalg.norm(goal_orientation - current_orientation)
        if orientation_diff < 0.1:
            reward += 0.5  # Reward for correct orientation

...
def compute_dense_reward(self, action) -> float:
    ...
    if grasp_success:
        ...
        reward += max(0, 1.0 - height_diff) * 0.5
    ...
    joint_vel_penalty = -0.001 * np.linalg.norm(self.agent.robot.get_qvel()[:-2])
    reward += joint_vel_penalty
    
    # Penalize joint position deviation from neutral position
    neutral_qpos = np.zeros_like(self.agent.robot.get_qpos()[:-2])
    joint_pos_penalty = -0.001 * np.linalg.norm(self.agent.robot.get_qpos()[:-2] - neutral_qpos)
